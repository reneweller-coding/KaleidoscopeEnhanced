 ...
 *   audioBeat      -> the zoom breathes (an envelope, continuous)
 *   audioMelodyPitch -> zoom into the field (high = closer)
...
void main()
{
    ...
    float zoom = (zoomP > 0.05 ? zoomP : 1.0) * mix(1.3, 0.6, clamp(audioMelodyPitch, 0.0, 1.0)) * (1.0 - 0.08 * audioBeat);
    ...
    vec3 col = wash + bodyCol * body * (0.7 + 0.5 * audioLevel) + edgeCol * edge * 0.8;
