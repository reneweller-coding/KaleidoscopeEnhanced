...
//rwrwtest
    ...
    float powerTunnelKaleidoscope = 1.01;
    // cartesian to polar coordinates
    //r = length(p);
    r = pow( pow(p.x*p.x,powerTunnelKaleidoscope) + pow(p.y*p.y,powerTunnelKaleidoscope), 1.0/(2*powerTunnelKaleidoscope) );
    a = atan(p.y, p.x);
...
//rwrwtest    
    ...
    float powerTunnel = 1.51;
  
    float a1 = atan(p.y,p.x);
    //float r1 = sqrt(dot(p,p));
    float r1 = pow( pow(p.x*p.x,powerTunnel) + pow(p.y*p.y,powerTunnel), 1.0/(2*powerTunnel) );

    
	vec2 uv;
    uv.x = (speedTunnel*time+.1/r1);
    uv.y = (a1/3.1416);
