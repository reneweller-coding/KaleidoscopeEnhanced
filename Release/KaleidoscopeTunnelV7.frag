...
//Tunnel foreward
    ...
    // kaleidoscope
    float sidesT2 = 0.5 * sidesTunnel;
    a = mod(a, tau/sidesT2);
    a = abs(a - tau/sidesT2/2.);
    a += time*speedKaleidoscopeTunnel; // rotate
    
	vec2 uv2;
    uv2.x = (speedTunnel*time*-0.5+.1/r);
    uv2.y = (a/3.1416);
    
    vec4 col2;
    col2 =  interpolation * texture2D(tex0,uv2) + (1.0-interpolation)*texture2D(tex1,uv2);
    color2 = 0.5*color2+0.5*col2;
