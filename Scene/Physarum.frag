...
void main()
{
    ...
    vec2 tr = texture2D(texPhysarum, suv).rg;

    // Relief lighting from the trail gradient (embossed living veins).
    vec2 px = vec2(2.0 / 512.0);
    float hC = tr.r + tr.g;
    float hX = texture2D(texPhysarum, suv + vec2(px.x, 0.0)).r
             + texture2D(texPhysarum, suv + vec2(px.x, 0.0)).g - hC;
    float hY = texture2D(texPhysarum, suv + vec2(0.0, px.y)).r
             + texture2D(texPhysarum, suv + vec2(0.0, px.y)).g - hC;
