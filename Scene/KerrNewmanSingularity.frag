...
void main() {
    ...
    for (int i = 0; i < 90; ++i) {
        ...
        if (diskDist < 0.12 && diskR > rHorizon * 1.2 && diskR < 4.8) {
            ...
            vec3 glowColor = mix(vec3(1.0, 0.3, 0.05), vec3(0.3, 0.7, 1.0), smoothstep(1.5, 3.0, doppler));
        ...
        if (jetHeight > rHorizon * 0.8 && jetRadius < (0.2 + 0.08 * jetHeight)) {
            ...
            vec3 jetCol = mix(vec3(0.1, 0.5, 1.0), vec3(0.9, 0.2, 1.0), fract(p.y * 2.0 - time * 4.0));
