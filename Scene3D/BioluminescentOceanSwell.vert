...
void main() {
    ...
    disp += gerstnerWave(worldXZ, normalize(vec2( 1.0,  0.2)), 0.30 * amp, 3.2, 0.6, n);
    disp += gerstnerWave(worldXZ, normalize(vec2( 0.7,  0.7)), 0.22 * amp, 2.1, 0.8, n);
    disp += gerstnerWave(worldXZ, normalize(vec2(-0.4,  0.9)), 0.15 * amp, 1.4, 1.1, n);
    disp += gerstnerWave(worldXZ, normalize(vec2( 0.9, -0.3)), 0.10 * amp, 0.8, 1.5, n);

    // Beat splash impulse
    float splash = sin(length(worldXZ) * 4.0 - time * 6.0) * audioKick * 0.4;
    disp.y += splash;

    vec3 pos = vec3(worldXZ.x + disp.x, disp.y - 0.8, worldXZ.y + disp.z);
    n = normalize(n);
