...
/**
 * @file DrumSkin.frag
 * @brief Lighting for the vibrating drumhead mesh generated by DrumSkin.comp:
 * a thin, tight, slightly iridescent mylar-film material lit by two key
 * lights plus a pair of tight/broad specular highlights, so the Bessel-mode
 * ripples read through the way the surface catches light rather than through
 * flat diffuse shading. Colour is keyed off the signed displacement (vDisp)
 * so crests and troughs read differently, with faint glowing nodal lines
 * where the surface is not moving, and a darker rim at the hoop (vRad).
 *
 * audioHigh sharpens the tight specular highlight, audioKick brightens the
 * nodal-line glow, audioBeat and audioSubBass give a final exposure pulse,
 * and audioAmbient adds a wash of the film colour; hueP and audioChromaHue
 * (via the house imgPalette arc) set the film's base hue.
 */
