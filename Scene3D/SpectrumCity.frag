...
/**
 * @file SpectrumCity.frag
 * @brief Shades the towers generated by SpectrumCity.comp as a dark night
 * city whose only real light source is its window grid, with concrete walls
 * barely more than a backdrop to hang the windows on.
 *
 * audioLevel and glowP set how many windows are lit, weighted per-building
 * by vEnergy (that tower's band energy); audioChromaHue tints the window
 * hue; audioKick drives a bright band sweeping up each tower; audioBeat
 * blinks the roof warning lights and pulses overall exposure; audioSubBass
 * washes the tallest towers (by vUp against heightP) in their band colour;
 * and audioAmbient warms the sky haze the towers fade into with distance
 * (vDist). camHP positions the view vector used for the sky-coloured rim
 * light.
 */
