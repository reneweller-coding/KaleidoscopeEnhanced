...
void main() {
    ...
    // Orbital radius and spherical orbital mechanics
    ...
    // Audio-reactive sail pitching & waving
    float pitchAngle = (0.2 + 0.3 * audioSwell) * sin(time * 2.0 + seed.x * 10.0);
    ...
    float glint = pow(max(dot(sailNormal, halfVec), 0.0), 32.0);
