...
/**
 * @file Magnetosphere.frag
 * @brief Lights the planet-and-dipole-field scene generated by
 * Magnetosphere.comp: a near-black planet lit only by a fixed sun and its
 * own aurora, surrounded by self-luminous plasma field-line tubes.
 *
 * vKind distinguishes planet (>1.5) from field line, and vEnergy (that
 * shell's audioSpectrum band, baked by the compute stage) drives the
 * tube's core-and-halo brightness and a white-hot centre; the shell
 * fraction (also vKind) picks the tube's hue from the photo palette so
 * inner shells read hot and outer ones cold. audioKick/audioHigh drive
 * the planet's polar aurora caps, audioLevel its limb atmosphere,
 * audioBeat/audioSubBass add a final overall pulse, and glowP scales
 * plasma brightness.
 */
...
void main()
{
    ...
    col *= 3.17;   // measured-dark lift (visual pass)
