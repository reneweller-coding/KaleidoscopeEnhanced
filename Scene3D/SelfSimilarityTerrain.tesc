#version 400 core
/**
 * @file SelfSimilarityTerrain.tesc
 * @brief Tessellation control for SelfSimilarityTerrain: adaptive by
 * distance, a little finer with the swell; fractional_odd_spacing keeps
 * level changes continuous; edge levels from the edge midpoints.
 */
...
float levelFor(vec2 uvA, vec2 uvB)
{
    ...
    return clamp(150.0 * detailP * fine / max(d, 2.0), 1.0, 24.0);
}
