...
{
	setWindowTitle( "Kaleidoscope Setup" );
	resize( 460, 640 );
	...
	// ---- Start ----
	auto *gStart = new QGroupBox( "Start" );
	auto *fStart = new QFormLayout( gStart );
	m_startConfig = new QComboBox();
	m_startConfig->addItem( "(zuletzt verwendet)", QString() );
	for( const QString &n : discoverConfigNames() )
		m_startConfig->addItem( n, n );
	fStart->addRow( "Startkonfiguration:", m_startConfig );
	m_remotePort = new QSpinBox();
	m_remotePort->setRange( 0, 65535 );
	m_remotePort->setSpecialValueText( "Aus" );
	fStart->addRow( "Web-Remote-Port:", m_remotePort );
	root->addWidget( gStart );

	// ---- Optionale Online-Extras ----
	auto *gOnline = new QGroupBox( "Optionale Online-Extras (Internetzugriff)" );
	auto *fOnline = new QFormLayout( gOnline );
	m_lyricsMode = new QComboBox();
	m_lyricsMode->addItem( "Aus" );
	m_lyricsMode->addItem( "Scroll" );
	m_lyricsMode->addItem( "Karaoke" );
	fOnline->addRow( "Songtexte:", m_lyricsMode );
	m_lyricsKinetic = new QCheckBox( "Zeilen-Einflug-Animation (Karaoke)" );
	fOnline->addRow( QString(), m_lyricsKinetic );
	m_artistImages = new QCheckBox( "Künstlerbilder anzeigen" );
	fOnline->addRow( QString(), m_artistImages );
	m_videoEnabled = new QCheckBox( "Musikvideo automatisch suchen (YouTube via yt-dlp)" );
	fOnline->addRow( QString(), m_videoEnabled );
	auto *videoHint = new QLabel(
	    "Braucht \"Künstlerbilder\" an (nutzt denselben Eck-Platz) und yt-dlp auf dem PATH." );
	...
	// ---- Verhalten ----
	auto *gBehav = new QGroupBox( "Verhalten" );
	auto *fBehav = new QFormLayout( gBehav );
	m_autoConfig = new QCheckBox( "Auto-Preset nach Stimmung" );
	fBehav->addRow( QString(), m_autoConfig );
	m_autoScale = new QCheckBox( "Adaptive Bildskalierung (FPS-abhängig)" );
	fBehav->addRow( QString(), m_autoScale );
	m_nowPlaying = new QCheckBox( "Titel-Einblendung bei Songwechsel" );
	fBehav->addRow( QString(), m_nowPlaying );
	m_lightShow = new QCheckBox( "Lightshow (Bühnenlampen-Overlay)" );
	fBehav->addRow( QString(), m_lightShow );
	root->addWidget( gBehav );

	// ---- Bild/Ton-Feintuning ----
	auto *gTune = new QGroupBox( "Bild/Ton-Feintuning" );
	...
	fTune->addRow( "Render-Skalierung:", m_renderScale );
	m_stereoMode = new QComboBox();
	m_stereoMode->addItem( "Aus" );
	m_stereoMode->addItem( "Side-by-Side" );
	m_stereoMode->addItem( "Top-Bottom" );
	m_stereoMode->addItem( "Anaglyph (Rot/Cyan)" );
	fTune->addRow( "Stereoskopie:", m_stereoMode );
	...
	fTune->addRow( "Stereo-Tiefe:", m_stereoDepth );
	root->addWidget( gTune );

	root->addStretch( 1 );
	scrollContent->setLayout( root );
	...
	auto *saveBtn  = new QPushButton( "Speichern" );
	...
	QObject::connect( saveBtn, &QPushButton::clicked, this, [this]() { saveToIni(); } );
...
void SetupWindow::loadFromIni()
{
	...
	if( !QFile::exists( path ) )
		m_status->setText( "Noch keine gespeicherten Einstellungen gefunden - Vorgabewerte." );
}
...
void SetupWindow::saveToIni()
{
	...
	{
		m_status->setStyleSheet( "color: #4a4;" );
		m_status->setText( "Gespeichert - wirkt beim nächsten Start von Kaleidoscope.exe." );
	}
	else
	{
		m_status->setStyleSheet( "color: #d55;" );
		m_status->setText( "Konnte nicht gespeichert werden: " + settingsPath() );
	}
